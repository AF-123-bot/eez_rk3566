...
void action_user_shutdown_reboot(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_PRESSED)
    {
        ...
        else if(obj == objects.btn_user_reboot) system("reboot");
        else if(obj == objects.btn_user_shutdown)   system("shutdown -h now");
    }
}
...
void action_btn_sample_motor(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_PRESSED)
    {
        ...
        {
            sample_motor_flag = 1;
            add_log_to_container("执行了一次手动取灰");
        ...
        {
            sample_motor_flag = 1;
            add_log_to_container("执行了一次手动清灰");
